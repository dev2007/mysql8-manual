...
要将构建密钥导入你的个人公共 GPG 密钥圈，请使用 **gpg--import**。例如，如果已将密钥保存在名为 `mysql_pubkey.asc` 的文件中，则导入命令如下所示：
...
你还可以使用公钥 id `5072E1F5`，从公钥服务器下载密钥：
...
如果要将密钥导入 RPM  配置以验证 RPM 安装包，则应能够直接导入密钥：
...
表格 2.1 源文件的 MySQL 包和签名文件

|文件类型|文件名|
|--|--|
|发布文件|mysql-standard-8.0.27-linux-i686.tar.gz|
|签名文件|mysql-standard-8.0.27-linux-i686.tar.gz.asc|

确保两个文件存储在同一目录中，然后运行以下命令验证分发文件的签名：

```bash
$> gpg --verify package_name.asc
```

如果下载的软件包有效，你应该会看到类似以下内容的 `Good signature` 消息：

```bash
$> gpg --verify mysql-standard-8.0.27-linux-i686.tar.gz.asc
gpg: Signature made Tue 01 Feb 2011 02:38:30 AM CST using DSA key ID 5072E1F5
gpg: Good signature from "MySQL Release Engineering <mysql-build@oss.oracle.com>"
```

`Good signature` 信息表明，与我们网站上列出的签名相比，文件签名是有效的。但你也可能会看到警告，如：
...
- *gpg*：*no ultimately trusted keys found*：这意味着特定密钥未被你或你的信任网“最终信任”，这对于验证文件签名来说是可以的。

- *WARNING*：*This key is not certified with a trusted signature! There is no indication that the signature belongs to the owner.*。注意：这是指你相信自己拥有我们真正的公钥的信任程度。这是个人的决定。理想情况下，MySQL 开发人员会亲自将密钥交给你，但更常见的情况是，你下载了密钥。下载被篡改了吗？可能不会，但这个决定取决于你。建立信任网是信任他们的一种方法。

有关如何使用公钥的更多信息，参阅 GPG 的文档。

> [原文链接](https://dev.mysql.com/doc/refman/8.0/en/checking-gpg-signature.html)
