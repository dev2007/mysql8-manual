...
作为一种流行的开源软件，MySQL 以其原始或重新打包的形式广泛安装在来自不同来源的许多系统上，包括不同的软件下载站点、软件存储库等。以下说明假设 MySQL 尚未使用第三方分布式 RPM 包安装在你的系统上；如果不是这样，请按照[章节 2.11.7，“使用 MySQL Yum 他库升级 MySQL”](/2/2.11/2.11.7/updating-yum-repo.html)或者[使用 MySQL Yum 仓库替换 MySQL 的第三方发行版](/2/2.5/2.5.2/replace-third-party-yum.html)中给出的说明进行操作。
